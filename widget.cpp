...
Widget::Widget(QWidget *parent): QWidget(parent){
  ...
  setWindowTitle("Calcolatore di matrici");
  //RIQUADRI PER LE MATRICI
  ...
  //OPERATORI BINARI (SOMMA, SOTTRAZIONE, PRODOTTO)
  ...
  activeMatComboBox->addItem(tr("Matrice sinistra"));
  activeMatComboBox->addItem(tr("Matrice destra"));
  ...
  typeMatComboBox->addItem(tr("Matrice standard"));
  typeMatComboBox->addItem(tr("Matrice quadrata"));
  typeMatComboBox->addItem(tr("Matrice binaria"));
...
void Widget::opSomma(){
  ...
  try{
    mat[matriceSelect]->writeMatrix(&(*mat[matriceSelect]->getMatrix()+*mat[notActiveMat()]->getMatrix()));
      ...
      display->setText("Non si possono sommare due matrici di dimensioni diverse");}
...
void Widget::opSottrazione(){
  ...
  try{
    mat[matriceSelect]->writeMatrix(&(*mat[matriceSelect]->getMatrix()-*mat[notActiveMat()]->getMatrix()));
      ...
      display->setText("Non si possono sottrarre due matrici di dimensioni diverse");}
  emit mat[matriceSelect]->update();
}
...
void Widget::opProdotto(){
  ...
  try{
    mat[matriceSelect]->writeMatrix(&(*mat[matriceSelect]->getMatrix()*(*mat[notActiveMat()]->getMatrix())));
      ...
      display->setText("Le dimensioni delle matrici non sono valide per un prodotto");}
  emit mat[matriceSelect]->update();
  updateDim();
}
...
void Widget::opQuadratoMagico(){
  ...
  try{
    ...
    display->setText("La matrice deve avere almeno 3 righe e 3 colonne");}
...
void Widget::opSudoku(){
  ...
  try{
    ...
    display->setText("Sono accettate matrici di dimensioni 4x4, 9x9, 16x16");}
  catch (errSudokuInit){
    display->setText("Il sudoku di partenza è errato");}
  mat[matriceSelect]->writeMatrix(m);
  emit mat[matriceSelect]->update();
}
...
void Widget::opLabirinto(){
  ...
  try{
    ...
    display->setText("Il labirinto è impossibile");}
  mat[matriceSelect]->writeMatrix(m);
